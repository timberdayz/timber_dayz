...
def build_distinct_raw_store_items(
        ...
        items.append(
            {
                ...
                "suggested_target_id": _generate_suggested_id(
                    account or "", site or "", store_label_raw or ""
                ),
...
async def get_missing_mappings(
    min_orders: int = 5, db: AsyncSession = Depends(get_async_db)
):
    """
    获取缺失映射建议
...
async def batch_add_aliases(request: Dict, db: AsyncSession = Depends(get_async_db)):
    ...
    try:
        ...
        success_count, failed_count = service.batch_add_aliases(
            mappings, created_by="web_ui_batch"
        )

        return BatchAddAliasesResponse(
            success=True, added=success_count, skipped=failed_count, errors=[]
        )
    except Exception as e:
...
async def backfill_alignment(
    limit: int = None, db: AsyncSession = Depends(get_async_db)
):
    """
    执行回填(将别名映射应用到历史订单)
...
async def import_yaml_aliases(
    ...
    try:
        ...
        service = get_account_alignment_service(db)
        success_count, failed_count = service.batch_add_aliases(
            mappings, created_by="yaml_import"
        )

        return ImportResponse(
            success=True, imported=success_count, skipped=failed_count, errors=[]
        )
...
async def import_csv_aliases(
    ...
    try:
        ...
        service = get_account_alignment_service(db)
        success_count, failed_count = service.batch_add_aliases(
            mappings, created_by="csv_import"
        )
...
async def export_yaml_aliases(
    ...
    try:
        ...
        yaml_content = yaml.dump(
            {"shop_aliases": shop_aliases}, allow_unicode=True, sort_keys=True
        )
...
async def update_alias(
    alias_id: int, request: Dict, db: AsyncSession = Depends(get_async_db)
):
    """
    更新别名映射
    ...
    try:
        result = await db.execute(
            select(AccountAlias).where(AccountAlias.id == alias_id)
        )
...
async def get_distinct_raw_stores(
    ...
    try:
        ...
        result = await db.execute(stmt, {"platform": platform, "domain": data_domain})
        stores = build_distinct_raw_store_items(result.all())

        return DistinctRawStoresResponse(success=True, stores=stores, count=len(stores))
    except Exception as e:
