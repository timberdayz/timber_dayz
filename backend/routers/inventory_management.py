...
async def get_products(
    ...
    try:
        logger.info(
            f"[GetProducts] 查询: platform={platform}, keyword={keyword}, category={category}"
        )
        ...
        if keyword:
            conditions.append(
                "(raw_data->>'SKU' ILIKE :keyword OR raw_data->>'商品名称' ILIKE :keyword)"
            )
        ...
        if not platforms:
            empty_payload = {
                ...
                "performance": {
                    "query_time_ms": round(
                        (time.perf_counter() - query_started_at) * 1000, 2
                    ),
        ...
        for platform_code, table_name in existing_platforms:
            union_queries.append(
                ...
                f'FROM b_class."{table_name}" '  # nosec B608
        ...
        for _platform_code, table_name in existing_platforms:
            count_queries.append(
                f'SELECT COUNT(*) FROM b_class."{table_name}" '  # nosec B608
                f"WHERE {where_clause}"  # nosec B608
            )
        ...
        for count_sql in count_queries:
            try:
                ...
                logger.warning(
                    f"查询表 {count_sql.split('FROM')[1].split('WHERE')[0].strip()} 失败(可能不存在): {e}"
                )
        ...
        for row in rows:
            ...
            product = {
                ...
                "available_stock": (
                    float(raw_data.get("可用库存", 0))
                    if raw_data.get("可用库存")
                    else 0
                ),
                "total_stock": (
                    float(raw_data.get("总库存", 0)) if raw_data.get("总库存") else 0
                ),
                "reserved_stock": (
                    float(raw_data.get("预留库存", 0))
                    if raw_data.get("预留库存")
                    else 0
                ),
                "in_transit_stock": (
                    float(raw_data.get("在途库存", 0))
                    if raw_data.get("在途库存")
                    else 0
                ),
        ...
        for product in mv_result["data"]:
            ...
            if platform_code and shop_id_value:
                ...
                result = await db.execute(
                    ...
                    .order_by(
                        ProductImage.is_main_image.desc(), ProductImage.image_order
                    )
                )
                ...
                result = await db.execute(
                    select(ProductImage)
                    .where(ProductImage.platform_sku == platform_sku)
                    .order_by(
                        ProductImage.is_main_image.desc(), ProductImage.image_order
                    )
                )
            ...
            inventory_data = {
                ...
                or main_image,  # v4.10.0新增:使用视图的image_url字段
                "image_count": (
                    len(images) if images else (1 if product.get("image_url") else 0)
                ),
                "all_images": all_images,
                # 时间戳
                "metric_date": (
                    product.get("metric_date").isoformat()
                    if product.get("metric_date")
                    else None
                ),
...
async def get_product_detail(
    ...
    try:
        logger.info(
            f"[GetProductDetail] 查询: sku={sku}, platform={platform}, shop_id={shop_id}"
        )
        ...
        detail = {
            ...
            "click_through_rate": (
                float(product.click_through_rate) if product.click_through_rate else 0
            ),
            "conversion_rate": (
                float(product.conversion_rate) if product.conversion_rate else 0
            ),
            # 图片列表
            "images": image_list,
            # 时间戳
            "metric_date": (
                product.metric_date.isoformat() if product.metric_date else None
            ),
...
async def upload_product_image(
    ...
    try:
        logger.info(
            f"[UploadImage] sku={sku}, platform={platform}, filename={file.filename}"
        )
        ...
        result = image_processor.process_product_image(
            image_data, sku, index=0  # 将自动根据现有图片数量调整
        )
...
async def delete_product_image(image_id: int, db: AsyncSession = Depends(get_async_db)):
    """删除产品图片"""
    try:
        ...
        try:
            original_path = Path(
                image.image_url.replace(
                    "/static/product_images/", "data/product_images/"
                )
            )
            thumbnail_path = Path(
                image.thumbnail_url.replace(
                    "/static/product_images/", "data/product_images/"
                )
            )
    ...
    try:
        ...
        query = select(FactProductMetric).join(
            ...
            and_(
                ...
                FactProductMetric.data_domain
                == latest_date_subq.c.data_domain,  # v4.10.0新增:匹配data_domain
        ...
        query = query.filter(
            or_(
                ...
                and_(
                    ...
                    FactProductMetric.platform_code.isnot(
                        None
        ...
        total_stock_query = select(
            func.sum(
                case(
                    (
                        FactProductMetric.available_stock.isnot(None),
                        FactProductMetric.available_stock,
                    ),
                    (
                        FactProductMetric.total_stock.isnot(None),
                        FactProductMetric.total_stock,
                    ),
        ...
        if not platform:
            ...
            platforms_result = await db.execute(
                select(FactProductMetric.platform_code)
                .where(FactProductMetric.platform_code.isnot(None))
                .distinct()
            )
                ...
                if plt:
                    plt_query_subq = (
                        select(
                            ...
                            func.max(FactProductMetric.metric_date).label(
                                "latest_date"
                            ),
                        )
                        .where(FactProductMetric.platform_code == plt)
                        .group_by(
                            FactProductMetric.platform_code,
                            FactProductMetric.shop_id,
                            FactProductMetric.platform_sku,
                    ...
                    plt_query = (
                        ...
                        .join(
                            ...
                            and_(
                                FactProductMetric.platform_code
                                == plt_query_subq.c.platform_code,
                                FactProductMetric.shop_id == plt_query_subq.c.shop_id,
                                FactProductMetric.platform_sku
                                == plt_query_subq.c.platform_sku,
                                FactProductMetric.data_domain
                                == plt_query_subq.c.data_domain,  # v4.10.0新增
                                FactProductMetric.metric_date
                                == plt_query_subq.c.latest_date,
                            ),
                        )
                        .where(
                            or_(
                    ...
                    plt_count_result = await db.execute(
                        select(func.count()).select_from(plt_query.subquery())
                    )
