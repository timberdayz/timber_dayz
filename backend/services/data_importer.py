...
from backend.models.database import (
    ...
    # [DELETED] v4.19.0: FactOrder, FactOrderItem 已删除,订单数据现在导入到 b_class.fact_{platform}_orders_{granularity}
    FactProductMetric,
    DimProduct,
    DataQuarantine,
    CatalogFile,
...
def upsert_orders_v2(db: Session, rows: List[Dict[str, Any]], file_record: Optional[Any] = None) -> int:
    ...
    for idx, r in enumerate(rows):
        ...
        # [TODO] v4.19.0: 订单数据导入需要改为导入到 b_class.fact_{platform}_orders_{granularity}
        # 当前使用 FactOrder 表(已废弃),需要实现新的导入逻辑
        # 暂时跳过订单导入,避免报错
        logger.warning(f"[UpsertOrders] FactOrder 表已删除,订单数据应导入到 b_class.fact_{core.get('platform_code', 'unknown')}_orders_daily,当前跳过导入: order_id={core.get('order_id')}")
        return
        
        # [DEPRECATED] 以下代码已注释,待实现新的导入逻辑后删除
        """
        if is_postgresql:
            try:
        ...
        # [DELETED] v4.19.0: FactOrderItem 表已删除,订单明细数据应导入到 b_class.fact_{platform}_orders_{granularity}
        # 以下订单明细导入逻辑已注释,待实现新的导入逻辑后删除
        """
        ...
        # [DELETED] v4.19.0: FactOrderItem 表已删除,订单明细数据应导入到 b_class.fact_{platform}_orders_{granularity}
        # 如果包含platform_sku,跳过订单明细入库(已废弃)
        if platform_sku and str(platform_sku).strip() and str(platform_sku).strip().lower() not in ['n/a', 'na', 'none', 'null', '']:
            # [DEPRECATED] 订单明细入库逻辑已废弃,FactOrderItem 表已删除
            # [TODO] 如需入库订单明细,请使用 b_class.fact_{platform}_orders_{granularity} 表
            pass
