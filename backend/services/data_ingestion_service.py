...
def normalize_field_name_for_domain(
    ...
    if (
        domain
        and domain.lower() == "orders"
        and field_name in ORDERS_EXPLICIT_FIELD_MAP
    ):
        return ORDERS_EXPLICIT_FIELD_MAP[field_name]

    if domain and domain.lower() == "orders":
        orders_rmb_match = re.match(
            r"^(.*?)[(（]\s*RMB\s*[)）]\s*$", field_name, flags=re.IGNORECASE
        )
        if orders_rmb_match:
...
class DataIngestionService:
    ...
    def _safe_resolve_path(self, file_path: str) -> str:
        ...
        if path_obj.is_absolute():
            ...
            if path_obj.exists():
                ...
                if any(
                    key in file_path_normalized
                    for key in ["data/raw", "data/input", "temp/outputs", "downloads"]
                ):
            ...
            for key_path in key_paths:
                if key_path in path_str:
                    ...
                    if resolved_path.exists():
                        logger.info(
                            f"[SafePath] 从绝对路径提取相对路径: {file_path} -> {resolved_path}"
                        )
            ...
            if allowed_dir_normalized in file_path_normalized:
                if project_root:
                    ...
                    if any(part in file_path_normalized for part in project_key_parts):
                        logger.info(
                            f"[SafePath] 通过额外检查允许路径: {file_path} (允许目录: {allowed_dir_str})"
                        )
                        return str(resolved)

        allowed_dirs_str = [str(d) for d in allowed_dirs_paths]
        raise ValueError(
            f"[SafePath] 文件路径不在允许的目录中: {file_path} (项目根目录: {project_root}, 允许目录: {allowed_dirs_str})"
        )
    ...
    async def ingest_data(
        ...
        header_columns: Optional[
            List[str]
        ] = None,  # [*] v4.6.0 DSS架构:原始表头字段列表
        deduplication_fields: Optional[
            List[str]
        ] = None,  # [*] v4.14.0新增:核心去重字段列表
        ...
        try:
            ...
            logger.info(
                f"[Ingest] 数据域: {domain}, header_row: {header_row} (0-based, Excel第{header_row+1}行)"
            )
            ...
            file_exists = await loop.run_in_executor(
                None, lambda: Path(safe_path).exists()
            )
            if not file_exists:
            ...
            file_size_mb = await loop.run_in_executor(
                None, lambda: Path(safe_path).stat().st_size / (1024 * 1024)
            )
            ...
            try:
                ...
                if file_size_mb > 10:
                    logger.info(
                        f"[Ingest] 大文件规范化完成(只处理关键列): {file_size_mb:.2f}MB"
                    )
            except Exception as norm_error:
                logger.warning(
                    f"[Ingest] 规范化失败(使用原始数据): {norm_error}", exc_info=True
                )
                normalization_report = {
                    "filled_columns": [],
                    "filled_rows": 0,
            ...
            if len(all_rows) == 0:
                ...
                return {
                    ...
                    "normalization_report": (
                        normalization_report
                        if "normalization_report" in locals()
                        else {}
                    ),
                }
            ...
            file_original_columns = list(
                df.columns.tolist()
            )  # 文件原始列名(包含货币代码)
            original_header_columns = (
                file_original_columns  # 始终使用文件原始列名用于追溯
            )
            ...
            logger.info(
                f"[Ingest] [DSS] 跳过字段映射,直接使用原始数据: {len(enhanced_rows)}行,{len(original_header_columns)}个字段"
            )

            # [*] 向后兼容:如果提供了mappings,记录日志但不使用(DSS架构不需要字段映射)
            if mappings:
                logger.info(
                    f"[Ingest] [DSS] 检测到mappings参数({len(mappings)}个),DSS架构将忽略,直接使用原始数据"
                )
            ...
            if file_record:
                ...
                if not file_record.platform_code and not platform:
                    logger.warning(
                        f"[Ingest] [v4.18.1] 文件级别platform_code为空,使用unknown"
                    )
            ...
            logger.info(
                f"[Ingest] [DSS] 跳过数据验证,所有{len(enhanced_rows)}行数据将直接进入去重和入库流程"
            )
            validation_result = {
                "errors": [],
                "warnings": [],
            ...
            logger.info(
                f"[Ingest] [DSS] 数据入库开始: file_id={file_id}, 总行数={len(valid_rows)}(DSS架构:跳过验证,所有数据入库)"
            )
                ...
                try:
                    ...
                    if file_record:
                        setattr(
                            raw_importer,
                            "file_date_from",
                            getattr(file_record, "date_from", None),
                        )
                        setattr(
                            raw_importer,
                            "file_date_to",
                            getattr(file_record, "date_to", None),
                        )
                    ...
                    logger.info(
                        f"[Ingest] [DSS] 开始计算data_hash: {len(valid_rows)}行"
                    )
                    hash_scope_values = {
                        "platform_code": (
                            file_platform_code
                            if "file_platform_code" in locals()
                            else platform
                        ),
                    ...
                    logger.info(
                        f"[Ingest] [DSS] data_hash计算完成: {len(data_hashes)}个哈希"
                    )
                    ...
                    if not header_columns_for_storage and valid_rows:
                        ...
                        logger.warning(
                            f"[Ingest] [DSS] 无法获取原始表头,使用数据键作为header_columns"
                        )
                    ...
                    if (
                        not sub_domain_value
                        and file_record
                        and hasattr(file_record, "sub_domain")
                    ):
                    ...
                    if domain.lower() == "services" and not sub_domain_value:
                        file_name = (
                            file_record.file_name if file_record else f"文件{file_id}"
                        )
                        error_msg = (
                    ...
                    logger.info(
                        f"[Ingest] [DSS] 开始批量插入到fact_raw_data_{table_name_suffix}表"
                    )
                    ...
                    if file_record and file_record.platform_code:
                        ...
                        not platform_code_for_table
                        or platform_code_for_table.strip() == ""
                    ):
                    ...
                    import_result = await raw_importer.async_batch_insert_raw_data(
                        ...
                        shop_id=(
                            getattr(file_record, "shop_id", None)
                            if file_record
                            else None
                        ),
                        file_id=file_id,
                    ...
                    if isinstance(import_result, dict):
                        imported = import_result.get("inserted", 0) + import_result.get(
                            "updated", 0
                        )
                        ...
                        skipped = 0
                        logger.info(
                            f"[Ingest] [DSS] 批量插入完成: {imported}行成功(表=fact_raw_data_{domain}_{granularity})"
                        )
                    ...
                    logger.error(
                        f"[Ingest] [DSS] RawDataImporter入库失败: {e}", exc_info=True
                    )
                    ...
                    import_result = (
                        None  # [*] v4.16.0修复:确保import_result被定义,避免后续访问错误
                    )

                    return {
                        "success": False,
                        "message": _format_sync_stage_error(
                            "raw_storage", raw_import_error
                        ),
                        "staged": 0,
                        "imported": 0,
                        "amount_imported": 0,
            ...
            try:
                ...
                # [*] 修复:处理数据库事务回滚问题
                logger.error(
                    f"[Ingest] 更新文件状态失败: {commit_error}", exc_info=True
                )
                try:
                    ...
                    logger.error(
                        f"[Ingest] 重试更新文件状态失败: {retry_error}", exc_info=True
                    )
                    await self.db.rollback()
            ...
            if extract_images and imported > 0:
                try:
                    from backend.tasks.image_extraction import (
            ...
            if imported > 0:
                try:
                    ...
                    logger.info(
                        f"[Ingest] 已触发DATA_INGESTED事件: domain={domain}, rows={imported}"
                    )
            ...
            return {
                ...
                "message": (
                    ...
                    else f"数据入库成功:暂存{staged}行,入库{imported}行(DSS架构:跳过验证,所有数据入库)"
                ),
                "staged": staged,
                "imported": imported,
