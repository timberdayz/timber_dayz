...
class RawDataImporter:
    ...
    def get_target_table_name(
        ...
        Returns:
            表名字符串
        """
        return self.table_manager.get_table_name(
            platform, data_domain, sub_domain, granularity
        )

    def _validate_metric_date_against_file_range(
        self, metric_date: Optional[date]
    ) -> None:
    ...
    def _validate_business_dates_against_file_range(
        ...
        self._validate_business_date_against_file_range(
    ...
    def extract_metric_date(
        self, row: Dict[str, Any], header_columns: List[str]
    ) -> Optional[date]:
        """
        从数据行中提取metric_date
        ...
        for field in date_field_candidates:
            ...
            for header_col in header_columns:
                if (
                    field.lower() in header_col.lower()
                    or header_col.lower() in field.lower()
                ):
    ...
    def _parse_single_date_value(self, value: str) -> tuple:
        ...
        if has_time:
            ...
            datetime_match = re.match(
                r"^(\d{1,4}[/-]\d{1,2}[/-]\d{1,4})\s*[/\s]*(\d{1,2}:\d{2}(?::\d{2})?)",
                s,
            )
            if datetime_match:
    ...
    def _parse_date_only(self, value: str) -> Optional[date]:
        ...
        try:
            from modules.services.smart_date_parser import (
                parse_date as smart_parse_date,
            )
    ...
    def batch_insert_raw_data(
        ...
        header_columns: Optional[
            List[str]
        ] = None,  # [*] v4.16.0更新:归一化的header_columns(用于动态列管理)
        currency_codes: Optional[
            List[Optional[str]]
        ] = None,  # [*] v4.15.0新增:货币代码列表
        sub_domain: Optional[str] = None,  # [*] v4.16.0新增:子类型(services域必须提供)
        original_header_columns: Optional[
            List[str]
        ] = None,  # [*] v4.16.0新增:原始header_columns(包含货币代码,用于保存到数据库)
        ...
        try:
            ...
            for i, (row, data_hash) in enumerate(zip(rows, data_hashes)):
                ...
                header_columns_for_storage = (
                    original_header_columns
                    if original_header_columns
                    else header_columns
                )
            ...
            if header_columns:
                try:
                    ...
                    added_columns = dynamic_column_manager.ensure_columns_exist(
                        table_name=table_name, header_columns=header_columns
                    )
            ...
            if is_upsert:
                ...
                try:
                    ...
                    if index_def:
                        ...
                        is_expression_index_for_query = (
                            "COALESCE" in index_def.upper() or "(" in index_def
                        )
                except Exception as e:
                    logger.debug(
                        f"[RawDataImporter] [v4.17.0] 检查索引类型失败(继续): {e}"
                    )
            ...
            system_fields = {
                ...
                "data_hash",
                "ingest_timestamp",
                "currency_code",  # [*] v4.15.0新增
                "sub_domain",
                "template_id",  # [*] v4.16.0新增:services域需要;v4.17.0新增:template_id
                "period_start_date",
                "period_end_date",  # [*] v4.18.0新增:日期范围
                "period_start_time",
                "period_end_time",  # [*] v4.18.0新增:时间范围(可选)
            }
            ...
            for record in insert_data:
                prepared_record = {
                    k: v for k, v in record.items() if k in system_fields
                }
            ...
            if True:  # 总是使用原始SQL(因为表是动态的)
                ...
                if is_expression_index:
                    ...
                    # 普通索引:使用列名列表
                    conflict_clause = (
                        "(platform_code, shop_id, data_domain, granularity, data_hash)"
                    )
                ...
                if is_upsert:
                    ...
                    for field in update_fields:
                        if field == "raw_data":
                            ...
                            update_clauses.append(
                                f"header_columns = EXCLUDED.header_columns"
                            )
                        else:
                    ...
                    update_clause = (
                        ", ".join(update_clauses)
                        if update_clauses
                        else "ingest_timestamp = EXCLUDED.ingest_timestamp"
                    )
                ...
                error_count = 0
                total_batches = (
                    len(insert_data_prepared) + BATCH_SIZE - 1
                ) // BATCH_SIZE
                ...
                for record in insert_data_prepared:
                    ...
                    if "raw_data" in record_copy and isinstance(
                        record_copy["raw_data"], dict
                    ):
                        record_copy["raw_data"] = json.dumps(
                            record_copy["raw_data"], ensure_ascii=False
                        )
                    elif "raw_data" in record_copy and record_copy["raw_data"] is None:
                        record_copy["raw_data"] = None

                    # 转换header_columns(list -> JSON字符串)
                    if "header_columns" in record_copy and isinstance(
                        record_copy["header_columns"], list
                    ):
                        record_copy["header_columns"] = json.dumps(
                            record_copy["header_columns"], ensure_ascii=False
                        )
                    elif (
                        "header_columns" in record_copy
                        and record_copy["header_columns"] is None
                    ):
                        record_copy["header_columns"] = None

                    # [*] v4.17.0新增:确保sub_domain字段存在(services域)
                    if (
                        data_domain.lower() == "services"
                        and sub_domain
                        and "sub_domain" not in record_copy
                    ):
                        ...
                        if (
                            ...
                            for i, record_copy in enumerate(batch_records):
                                try:
                                    ...
                                    if (
                                        "unique" in row_error_str
                                        or "duplicate" in row_error_str
                                    ):
                                        batch_skipped += 1
                                    else:
                                        batch_errors += 1
                ...
                # 验证:确保所有行都被处理
                if processed_count + skipped_count + error_count != len(
                    insert_data_prepared
                ):
                    logger.warning(
            ...
            if header_columns and rows:
                try:
                    dynamic_column_manager = get_dynamic_column_manager(self.db)
                    existing_columns = dynamic_column_manager.get_existing_columns(
                        table_name
                    )
                    ...
                    for original_col in header_columns:
                        normalized_col = dynamic_column_manager.normalize_column_name(
                            original_col
                        )
                        # 只映射那些确实存在于表中的列(排除系统字段)
                        if (
                            normalized_col in existing_columns
                            and normalized_col not in system_fields
                        ):
                            column_mapping.setdefault(normalized_col, []).append(
                                original_col
                            )
                    ...
                    if column_mapping:
                        ...
                        for i, row in enumerate(rows):
                            ...
                            if data_domain.lower() == "services" and sub_domain:
                                ...
                                where_conditions.append(
                                    "COALESCE(shop_id, '') = COALESCE(:shop_id, '')"
                                )
                            ...
                            for (
                                ...
                                if chosen_value is not None:
                                    set_clauses.append(
                                        f'"{normalized_col}" = :{normalized_col}'
                                    )
            ...
            if is_upsert:
                ...
                if existing_hashes:
                    ...
                    new_hashes = {
                        record["data_hash"] for record in insert_data_prepared
                    }
                    ...
                    inserted_count = sum(
                        1
                        for r in insert_data_prepared
                        if r["data_hash"] in inserted_hashes
                    )
                    updated_count = sum(
                        1
                        for r in insert_data_prepared
                        if r["data_hash"] in updated_hashes
                    )
                else:
                    # 没有已存在的数据:全部是新插入
    ...
    async def async_batch_insert_raw_data(
        ...
        def _sync_batch_insert():
            ...
            try:
                ...
                sync_importer.file_date_from = file_date_from or getattr(
                    self, "file_date_from", None
                )
                sync_importer.file_date_to = file_date_to or getattr(
                    self, "file_date_to", None
                )
