...
class _FakeExportResult:
    def __init__(self):
        ...
        self.file_path = "downloads/shopee/acc/shop/products/monthly/export.xlsx"
...
class _FakeExportComponent:
    ...
    async def run(self, page):
        assert self.ctx.config["data_domain"] == "products"
        assert self.ctx.config["session_ready"] is True
        return _FakeExportResult()


@pytest.mark.asyncio
async def test_shopee_login_plus_products_export_combo_uses_shared_context():
    adapter = create_adapter(
        platform="shopee",
        account={"username": "u", "password": "p"},
        config={"data_domain": "products"},
    ...
    export_result = await adapter.export(page=object(), data_domain="products")

    assert login_result.success is True
    assert login_result.message == "logged-in"
    assert export_result.success is True
    assert export_result.file_path == "downloads/shopee/acc/shop/products/monthly/export.xlsx"
