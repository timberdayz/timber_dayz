...
def test_validate_collection_profile_requires_cloud_sync_values(monkeypatch):
    ...
    assert ok is False
    assert "ENABLE_COLLECTION 必须为 true" in errors
    assert "CLOUD_SYNC_WORKER_ENABLED 必须为 true" in errors
    assert "CLOUD_DATABASE_URL 未配置，自动云端同步无法启用" in errors
    assert "CLOUD_SYNC_TUNNEL_HOST 未配置" in errors
    assert "CLOUD_SYNC_TUNNEL_PORT 未配置" in errors
