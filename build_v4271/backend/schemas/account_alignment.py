"""
账号对齐相关的 Pydantic Schemas
用于账号别名管理、对齐统计、导入导出等 API
"""
...
class MappingSuggestion(BaseModel):
    ...
    first_order_date: Optional[str] = Field(None, description="最早订单日期")
    last_order_date: Optional[str] = Field(None, description="最近订单日期")
    suggested_target_id: Optional[str] = Field(None, description="建议的账号ID")
...
class DistinctRawStoreItem(BaseModel):
    """未对齐店铺项"""
    ...
    store_label_raw: str = Field(description="原始店铺标签")
    order_count: int = Field(description="订单数")
    total_gmv: float = Field(description="GMV")
    suggested_target_id: Optional[str] = Field(None, description="建议的账号ID")
    first_order: Optional[str] = Field(None, description="最早订单日期")
    last_order: Optional[str] = Field(None, description="最近订单日期")
...
class DistinctRawStoresResponse(BaseModel):
    ...
    success: bool = Field(default=True)
    stores: List[DistinctRawStoreItem] = Field(description="店铺列表")
