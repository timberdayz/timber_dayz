...
class _CustomRangePage:
    def __init__(self, *, displayed_start: str, displayed_end: str, confirm_clickable: bool = True) -> None:
        self.start_date = _FakeLocator()
        self.start_time = _FakeLocator()
        self.end_date = _FakeLocator()
        self.end_time = _FakeLocator()
        self.confirm = _FakeLocator()
        self.confirm_clickable = confirm_clickable
        self.range_start = _FakeLocator()
        self.range_end = _FakeLocator()
    ...
    def locator(self, selector: str):  # noqa: ANN001
        if selector in (
            ...
            return _FakeLocatorList([self.range_start, self.range_end])
    ...
    def get_by_role(self, role: str, name=None):  # noqa: ANN001
        ...
        if role == "textbox" and name == "结束时间":
            return self.end_time
        if role == "button" and name == "确定":
            if not self.confirm_clickable:
                raise AssertionError("confirm button not available")
            return self.confirm
        if role == "combobox" and name == "开始时间":
...
@pytest.mark.asyncio
async def test_miaoshou_date_picker_custom_range_prefers_range_inputs_and_clicks_confirm() -> None:
    component = MiaoshouDatePicker(_ctx())
    page = _CustomRangePage(
        displayed_start="2026-02-01 00:00",
        displayed_end="2026-02-28 23:59",
    )
...
@pytest.mark.asyncio
async def test_miaoshou_date_picker_custom_range_succeeds_when_range_inputs_apply_and_confirm_is_already_gone() -> None:
    component = MiaoshouDatePicker(_ctx())
    page = _CustomRangePage(
        displayed_start="2026-03-01 00:00",
        displayed_end="2026-03-31 23:59",
