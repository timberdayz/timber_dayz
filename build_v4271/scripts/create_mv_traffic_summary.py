...
# -*- coding: utf-8 -*-
"""
创建mv_traffic_summary物化视图（traffic域主视图）

使用方法：
    python scripts/create_mv_traffic_summary.py
"""
...
def create_mv_traffic_summary():
    """创建mv_traffic_summary物化视图"""
    safe_print("=" * 70)
    safe_print("创建mv_traffic_summary物化视图")
    safe_print("=" * 70)
    
    # 读取SQL文件
    sql_file = project_root / "sql" / "materialized_views" / "create_mv_traffic_summary.sql"
    ...
    try:
        ...
        result = db.execute(text("""
            SELECT COUNT(*) 
            FROM pg_matviews 
            WHERE matviewname = 'mv_traffic_summary'
        """)).scalar()
        
        if result > 0:
            row_count = db.execute(text("SELECT COUNT(*) FROM mv_traffic_summary")).scalar()
...
if __name__ == '__main__':
    success = create_mv_traffic_summary()
