...
**重要约定（按周期汇总时必读）**：在 Orders Model / Analytics Model 中，**无论粒度为 daily/weekly/monthly**，`metric_date`、`period_start_date`、`period_end_date` 在底层数据里通常存储的是**单日日期**（订单日期或事件日期），**不是**周期起止。因此：
- 按周/月汇总时，应**先根据参数计算周期范围**（如当周周一～周日、当月 1～月末），再用 **`metric_date` 落在该范围内** 做过滤，例如 `metric_date >= current_period_start AND metric_date <= current_period_end`。
- **勿**用 `period_start_date = 某周周一` 或 `period_end_date >= target_date` 等方式直接匹配周期，否则会导致月度/周度数据为 0 或跳变。
...
| `sales_amount`                | NUMERIC   | 销售额/销售金额/GMV/订单金额/成交金额         | sales_amount                  | 销售金额（若源数据常为 0，汇总时用 paid_amount） |
| `paid_amount`                 | NUMERIC   | 实付金额/买家实付金额                         | paid_amount                   | 买家实付金额；**按金额汇总时优先使用**（与 sales_amount 二选一） |
...
| `order_count`                 | NUMERIC   | 订单数/订单数量                               | order_count                   | 订单数量                   |
| `product_quantity`            | NUMERIC   | 产品数量/商品数量/数量/件数/销售数量/出库数量 | product_quantity/quantity/qty | 商品件数（用于连带率计算） |
