...
**总成本中的 A 类运营成本仅来自本表**。当前费用管理链路中，相关人工录入列的业务语义已统一为**营销费用**，不再作为正式员工工资录入入口。正式员工工资仅通过工资单链路计算与结算。为兼容历史数据库结构，底层 ORM 属性仍为 `salary`，数据库列仍为“工资”，但公开业务语义按“营销费用”解释，避免与工资单口径重复。
...
| 工资（业务语义：营销费用） | salary | numeric(15,2) | 兼容历史列名保留，当前业务语义为营销费用 |
...
- **单条运营成本** = 租金 + 营销费用 + 水电费 + 其他成本（当前四列之和；底层仍映射历史“工资”列）；若后续扩展新列（见下），则单条 = **所有成本列之和**。
