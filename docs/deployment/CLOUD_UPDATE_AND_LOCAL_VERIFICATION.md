...
3. Phase 2：`alembic upgrade heads`（若本地库与迁移链不一致会 WARN，可继续）  
...
**说明**：若出现 `Can't locate revision identified by 'xxx'`，表示当前库的迁移链与代码中 `migrations/versions` 不一致，需在本地执行 `alembic history` 与 `alembic current` 排查；脚本不会因此退出，仍会继续后续阶段。
