...
test('collection api exposes current-config workbench endpoints', () => {
  ...
  assert.equal(apiSource.includes('bulkAdvanceCurrentGranularity'), true)
...
test('collection config workbench renders main-account current-config shop layout', () => {
  ...
  assert.equal(viewSource.includes('advanceCurrentConfig'), true)
  assert.equal(viewSource.includes('bulkAdvanceCurrentGranularity'), true)
