...
export default {
  ...
  async putHrShopCommissionConfig(platformCode, shopId, data) {
    return await this._put(
      `/hr/shop-commission-config/${platformCode}/${shopId}`,
      data,
    );
  },
  ...
  async copyHrEmployeeShopAssignmentsFromPrevMonth(data) {
    return await this._post(
      "/hr/employee-shop-assignments/copy-from-prev-month",
      data,
    );
  },
  ...
  async getTargetByMonth(month, targetType = "shop") {
    return await this._get("/targets/by-month", {
      params: { month, target_type: targetType },
    });
  },
  ...
  async generateDailyBreakdown(targetId, options = {}) {
    return await this._post(
      `/targets/${targetId}/breakdown/generate-daily`,
      options,
    );
  },
