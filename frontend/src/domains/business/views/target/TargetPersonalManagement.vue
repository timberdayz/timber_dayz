  ...
  <div class="personal-workbench erp-page-container erp-page--admin">
    ...
    <template v-else>
      ...
      <section v-show="activeStep === 0" class="step-panel">
        <div class="panel-heading"><div><h2>启用个人考核项</h2><p>只选择本月启用项；系统按目录顺序自动均分个人运营 20 分。</p></div><el-button type="primary" :icon="Check" :disabled="!enabledCount" :loading="savingRules" @click="saveRules">保存规则</el-button></div>
        <section class="score-summary"><span>已启用 {{ enabledCount }} 项</span><span>自动分配 {{ assignedScore }} / 20 分</span></section>
        <el-table :data="metrics" border stripe class="erp-table">
          ...
          <el-table-column prop="metric_direction" label="评分方向" width="130" />
          ...
          <el-table-column prop="guidance" label="录入及评分说明" min-width="280" />
        </el-table>
      </section>
