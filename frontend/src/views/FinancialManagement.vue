<template>
  <div class="financial-management">
    <div class="page-header">
      <h1>财务结算中心</h1>
      <p>统一处理公司月结总账、利润分配基准、跟投收益试算、跟投记录与结算台账。</p>
    </div>
    <el-alert class="page-alert" type="info" :closable="false" show-icon title="建议先确认利润基准、店铺提成、工资和跟投审批，再进行月度利润结算审批。" />

    <el-tabs v-model="activeTab" class="finance-tabs">
      <el-tab-pane label="月度利润结算中心" name="monthly">
        <el-card class="section-card">
          <template #header><div class="card-header"><span>公司月结总账</span><el-tag type="success">company monthly settlement</el-tag></div></template>
          <el-form :inline="true" class="filter-form">
            <el-form-item label="月份"><el-date-picker v-model="monthlyForm.period_month" type="month" value-format="YYYY-MM" format="YYYY-MM" style="width: 140px" /></el-form-item>
            <el-form-item label="人员目标"><el-input-number v-model="monthlyForm.personnel_target_ratio" :min="0" :max="1" :step="0.05" :precision="2" /></el-form-item>
            <el-form-item label="跟投目标"><el-input-number v-model="monthlyForm.follow_target_ratio" :min="0" :max="1" :step="0.05" :precision="2" /></el-form-item>
            <el-form-item label="公司目标"><el-input-number v-model="monthlyForm.company_target_ratio" :min="0" :max="1" :step="0.05" :precision="2" /></el-form-item>
            <el-form-item label="调整金额"><el-input-number v-model="monthlyForm.adjustment_amount" :step="100" :precision="2" /></el-form-item>
            <el-form-item label="调整原因"><el-input v-model="monthlyForm.adjustment_reason" placeholder="请输入调整原因" style="width: 220px" /></el-form-item>
            <el-form-item>
          ...
          <el-row v-if="monthlySummary" :gutter="20" class="summary-grid">
            <el-col :xs="24" :sm="12" :md="6"><div class="summary-item"><div class="summary-label">月度净利润</div><div class="summary-value profit">{{ formatCurrency(monthlySummary.net_profit_amount) }}</div></div></el-col>
            <el-col :xs="24" :sm="12" :md="6"><div class="summary-item"><div class="summary-label">人员成本</div><div class="summary-value cost">{{ formatCurrency(monthlySummary.personnel_actual_amount) }}</div></div></el-col>
            <el-col :xs="24" :sm="12" :md="6"><div class="summary-item"><div class="summary-label">跟投收益</div><div class="summary-value revenue">{{ formatCurrency(monthlySummary.follow_actual_amount) }}</div></div></el-col>
            <el-col :xs="24" :sm="12" :md="6"><div class="summary-item"><div class="summary-label">公司留存</div><div class="summary-value profit">{{ formatCurrency(monthlySummary.company_actual_amount) }}</div></div></el-col>
          </el-row>
          <el-row v-if="monthlySummary" :gutter="20" class="summary-grid">
            <el-col :xs="24" :sm="12" :md="8"><div class="summary-item compact"><div class="summary-label">人员目标 / 实际</div><div>{{ formatCurrency(monthlySummary.personnel_target_amount) }} / {{ formatCurrency(monthlySummary.personnel_actual_amount) }}</div></div></el-col>
            <el-col :xs="24" :sm="12" :md="8"><div class="summary-item compact"><div class="summary-label">跟投目标 / 实际</div><div>{{ formatCurrency(monthlySummary.follow_target_amount) }} / {{ formatCurrency(monthlySummary.follow_actual_amount) }}</div></div></el-col>
            <el-col :xs="24" :sm="12" :md="8"><div class="summary-item compact"><div class="summary-label">公司目标 / 实际</div><div>{{ formatCurrency(monthlySummary.company_target_amount) }} / {{ formatCurrency(monthlySummary.company_actual_amount) }}</div></div></el-col>
          </el-row>
          <el-row v-if="monthlySummary" :gutter="20" class="summary-grid">
            <el-col :xs="24" :sm="12" :md="8"><div class="summary-item compact"><div class="summary-label">差异金额</div><div>{{ formatCurrency(monthlySummary.difference_amount) }}</div></div></el-col>
            <el-col :xs="24" :sm="12" :md="8"><div class="summary-item compact"><div class="summary-label">差异比例</div><div>{{ formatPercentValue(monthlySummary.difference_ratio) }}</div></div></el-col>
            <el-col :xs="24" :sm="12" :md="8"><div class="summary-item compact"><div class="summary-label">当前状态</div><div>{{ monthlySummary.status || '-' }}</div></div></el-col>
          </el-row>
          <el-divider>人员成本明细</el-divider>
          <el-table v-if="monthlyPersonnelDetails.length" :data="monthlyPersonnelDetails" stripe>
            <el-table-column prop="detail_type" label="人员成本类型" min-width="180" />
            <el-table-column prop="employee_code" label="员工编号" width="140" />
            <el-table-column prop="amount" label="金额" width="160" align="right"><template #default="{ row }">{{ formatCurrency(row.amount) }}</template></el-table-column>
            <el-table-column prop="source_module" label="来源模块" min-width="160" />
          </el-table>
          <el-empty v-else description="当前月份暂无人员成本明细" />
        </el-card>
      </el-tab-pane>
      <el-tab-pane label="统一利润分配基准" name="basis">
        <el-card class="section-card">
          <template #header><div class="card-header"><span>店铺结算净利润口径</span><el-tag type="info">profit_basis_amount</el-tag></div></template>
          <el-alert type="info" :closable="false" show-icon class="section-alert" title="订单利润用于经营分析，profit_basis_amount 才是正式结算和利润分配的唯一基准。" />
          <el-form :inline="true" class="filter-form">
            ...
            <el-form-item label="平台"><el-select v-model="profitBasisForm.platform_code" style="width: 140px"><el-option label="Shopee" value="shopee" /><el-option label="TikTok" value="tiktok" /><el-option label="Amazon" value="amazon" /><el-option label="妙手ERP" value="miaoshou" /></el-select></el-form-item>
            <el-form-item label="店铺ID"><el-input v-model="profitBasisForm.shop_id" placeholder="请输入店铺ID" style="width: 180px" /></el-form-item>
            <el-form-item><el-button type="primary" @click="loadProfitBasis" :loading="financeStore.profitBasis.loading">查询基准</el-button><el-button @click="rebuildProfitBasis" :loading="financeStore.profitBasis.loading">重算基准</el-button></el-form-item>
          </el-form>
      ...
      <el-tab-pane label="跟投收益试算" name="trial">
        <el-card class="section-card">
          <template #header><div class="card-header"><span>跟投收益试算</span></div></template>
          <el-form :inline="true" class="filter-form">
            ...
            <el-form-item label="平台"><el-select v-model="followInvestmentForm.platform_code" style="width: 140px"><el-option label="Shopee" value="shopee" /><el-option label="TikTok" value="tiktok" /><el-option label="Amazon" value="amazon" /><el-option label="妙手ERP" value="miaoshou" /></el-select></el-form-item>
            <el-form-item label="店铺ID"><el-input v-model="followInvestmentForm.shop_id" placeholder="请输入店铺ID" style="width: 180px" /></el-form-item>
            ...
            <el-form-item><el-button type="primary" @click="runFollowInvestmentSettlement" :loading="financeStore.followInvestmentSettlement.loading">试算收益</el-button></el-form-item>
          </el-form>
      ...
      <el-tab-pane label="跟投记录" name="records">
        <el-card class="section-card">
          <template #header><div class="card-header"><span>跟投记录</span><el-button type="primary" size="small" @click="openCreateFollowInvestment">新增跟投</el-button></div></template>
          <el-form :inline="true" class="filter-form">
            <el-form-item label="平台"><el-select v-model="followInvestmentQuery.platform_code" style="width: 140px"><el-option label="Shopee" value="shopee" /><el-option label="TikTok" value="tiktok" /><el-option label="Amazon" value="amazon" /><el-option label="妙手ERP" value="miaoshou" /></el-select></el-form-item>
            <el-form-item label="店铺ID"><el-input v-model="followInvestmentQuery.shop_id" placeholder="请输入店铺ID" style="width: 180px" /></el-form-item>
            ...
            <el-form-item><el-button @click="loadFollowInvestments" :loading="financeStore.followInvestments.loading">查询记录</el-button></el-form-item>
          </el-form>
      ...
      <el-tab-pane label="结算台账" name="ledger">
        <el-card class="section-card">
          <template #header><div class="card-header"><span>跟投结算台账</span></div></template>
          <el-form :inline="true" class="filter-form">
            ...
            <el-form-item label="平台"><el-select v-model="settlementQuery.platform_code" style="width: 140px"><el-option label="Shopee" value="shopee" /><el-option label="TikTok" value="tiktok" /><el-option label="Amazon" value="amazon" /><el-option label="妙手ERP" value="miaoshou" /></el-select></el-form-item>
            <el-form-item label="店铺ID"><el-input v-model="settlementQuery.shop_id" placeholder="请输入店铺ID" style="width: 180px" /></el-form-item>
            ...
            <el-form-item><el-button @click="loadFollowInvestmentSettlements" :loading="financeStore.followInvestmentSettlements.loading">查询台账</el-button></el-form-item>
          </el-form>
    ...
    <el-dialog v-model="showFollowInvestmentDialog" :title="followInvestmentDialogMode === 'create' ? '新增跟投' : '编辑跟投'" width="600px">
      <el-form :model="followInvestmentRecordForm" label-width="120px">
        ...
        <el-form-item label="平台"><el-select v-model="followInvestmentRecordForm.platform_code" style="width: 180px"><el-option label="Shopee" value="shopee" /><el-option label="TikTok" value="tiktok" /><el-option label="Amazon" value="amazon" /><el-option label="妙手ERP" value="miaoshou" /></el-select></el-form-item>
        <el-form-item label="店铺ID"><el-input v-model="followInvestmentRecordForm.shop_id" /></el-form-item>
...
const loadProfitBasis = async () => { if (!profitBasisForm.value.shop_id) return ElMessage.warning('请输入店铺ID'); await financeStore.fetchProfitBasis({ period_month: profitBasisForm.value.period_month, platform_code: profitBasisForm.value.platform_code, shop_id: profitBasisForm.value.shop_id }) }
const rebuildProfitBasis = async () => { if (!profitBasisForm.value.shop_id) return ElMessage.warning('请输入店铺ID'); try { await financeStore.rebuildProfitBasis({ period_month: profitBasisForm.value.period_month, platform_code: profitBasisForm.value.platform_code, shop_id: profitBasisForm.value.shop_id, basis_version: 'A_ONLY_V1' }); ElMessage.success('利润分配基准已重算') } catch (error) { ElMessage.error('重算利润基准失败: ' + error.message) } }
const runFollowInvestmentSettlement = async () => { if (!followInvestmentForm.value.shop_id) return ElMessage.warning('请输入店铺ID'); try { await financeStore.calculateFollowInvestmentSettlement({ period_month: followInvestmentForm.value.period_month, platform_code: followInvestmentForm.value.platform_code, shop_id: followInvestmentForm.value.shop_id, distribution_ratio: followInvestmentForm.value.distribution_ratio }); ElMessage.success('跟投收益试算完成') } catch (error) { ElMessage.error('跟投收益试算失败: ' + error.message) } }
const loadFollowInvestments = async () => { await financeStore.fetchFollowInvestments({ platform_code: followInvestmentQuery.value.platform_code || undefined, shop_id: followInvestmentQuery.value.shop_id || undefined, status: followInvestmentQuery.value.status || undefined }) }
const loadFollowInvestmentSettlements = async () => { await financeStore.fetchFollowInvestmentSettlements({ period_month: settlementQuery.value.period_month || undefined, platform_code: settlementQuery.value.platform_code || undefined, shop_id: settlementQuery.value.shop_id || undefined, status: settlementQuery.value.status || undefined }) }
...
const resetFollowInvestmentRecordForm = () => { followInvestmentRecordForm.value = { investor_user_id: null, platform_code: 'shopee', shop_id: '', contribution_amount: 0, contribution_date: new Date().toISOString().split('T')[0], withdraw_date: '', status: 'active', remark: '' }; editingFollowInvestmentId.value = null }
...
const submitFollowInvestmentRecord = async () => { try { submitting.value = true; const payload = { ...followInvestmentRecordForm.value, withdraw_date: followInvestmentRecordForm.value.withdraw_date || null }; if (followInvestmentDialogMode.value === 'create') { await financeStore.createFollowInvestment(payload); ElMessage.success('跟投记录已创建') } else { await financeStore.updateFollowInvestment(editingFollowInvestmentId.value, payload, { platform_code: followInvestmentQuery.value.platform_code || undefined, shop_id: followInvestmentQuery.value.shop_id || undefined, status: followInvestmentQuery.value.status || undefined }); ElMessage.success('跟投记录已更新') } showFollowInvestmentDialog.value = false; resetFollowInvestmentRecordForm() } catch (error) { ElMessage.error('保存跟投记录失败: ' + error.message) } finally { submitting.value = false } }
...
onMounted(() => { loadMonthlySettlement(); loadFollowInvestments(); loadFollowInvestmentSettlements() })
</script>
