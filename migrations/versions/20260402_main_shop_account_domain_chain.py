...
def upgrade() -> None:
    ...
    if "main_accounts" not in table_names:
        op.create_table(
            "main_accounts",
    ...
    main_account_indexes = _index_names(bind, "main_accounts")
    if "ix_main_accounts_platform" not in main_account_indexes:
        op.create_index("ix_main_accounts_platform", "main_accounts", ["platform"], schema=SCHEMA_NAME)
    if "ix_main_accounts_enabled" not in main_account_indexes:
        op.create_index("ix_main_accounts_enabled", "main_accounts", ["enabled"], schema=SCHEMA_NAME)

    if "shop_accounts" not in table_names:
        op.create_table(
            "shop_accounts",
            sa.Column("id", sa.Integer(), primary_key=True, autoincrement=True),
            sa.Column("platform", sa.String(length=50), nullable=False),
    ...
    shop_account_indexes = _index_names(bind, "shop_accounts")
    if "ix_shop_accounts_main_account_id" not in shop_account_indexes:
        op.create_index("ix_shop_accounts_main_account_id", "shop_accounts", ["main_account_id"], schema=SCHEMA_NAME)
    if "ix_shop_accounts_platform_shop_id" not in shop_account_indexes:
        op.create_index("ix_shop_accounts_platform_shop_id", "shop_accounts", ["platform_shop_id"], schema=SCHEMA_NAME)
    if "ix_shop_accounts_enabled" not in shop_account_indexes:
        op.create_index("ix_shop_accounts_enabled", "shop_accounts", ["enabled"], schema=SCHEMA_NAME)

    if "shop_account_aliases" not in table_names:
        op.create_table(
    ...
    shop_alias_indexes = _index_names(bind, "shop_account_aliases")
    if "ix_shop_account_aliases_shop_account_id" not in shop_alias_indexes:
        op.create_index("ix_shop_account_aliases_shop_account_id", "shop_account_aliases", ["shop_account_id"], schema=SCHEMA_NAME)
    if "ix_shop_account_aliases_platform_alias_normalized" not in shop_alias_indexes:
        op.create_index("ix_shop_account_aliases_platform_alias_normalized", "shop_account_aliases", ["platform", "alias_normalized"], schema=SCHEMA_NAME)

    if "shop_account_capabilities" not in table_names:
        op.create_table(
    ...
    capability_indexes = _index_names(bind, "shop_account_capabilities")
    if "ix_shop_account_capabilities_shop_account_id" not in capability_indexes:
        op.create_index("ix_shop_account_capabilities_shop_account_id", "shop_account_capabilities", ["shop_account_id"], schema=SCHEMA_NAME)

    if "platform_shop_discoveries" not in table_names:
        op.create_table(
    ...
    discovery_indexes = _index_names(bind, "platform_shop_discoveries")
    if "ix_platform_shop_discoveries_main_account_id" not in discovery_indexes:
        op.create_index("ix_platform_shop_discoveries_main_account_id", "platform_shop_discoveries", ["main_account_id"], schema=SCHEMA_NAME)
    if "ix_platform_shop_discoveries_status" not in discovery_indexes:
        op.create_index("ix_platform_shop_discoveries_status", "platform_shop_discoveries", ["status"], schema=SCHEMA_NAME)
    ...
    bind.execute(
        sa.text(
            ...
            SELECT
                ...
                pa.updated_by
            FROM core.platform_accounts pa
            ON CONFLICT (shop_account_id) DO NOTHING
            """
        )
    )
    ...
    bind.execute(
        sa.text(
            ...
            ON CONFLICT ON CONSTRAINT uq_shop_account_capabilities_domain DO NOTHING
            """
        )
    )
