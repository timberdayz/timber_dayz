...
def upgrade() -> None:
    ...
    if "employee_tasks" not in existing_tables:
        ...
        op.create_index("ix_employee_tasks_owner_status", "employee_tasks", ["owner_user_id", "status"], unique=False)
        op.create_index("ix_employee_tasks_due_at", "employee_tasks", ["due_at"], unique=False)
        op.create_index(
        ...
        op.create_index("ix_employee_tasks_created_at", "employee_tasks", ["created_at"], unique=False)
    ...
    if "employee_task_logs" not in existing_tables:
        op.create_table(
            ...
            sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("now()")),
            sa.ForeignKeyConstraint(["task_pk"], ["employee_tasks.id"], ondelete="CASCADE"),
            sa.ForeignKeyConstraint(["actor_user_id"], ["core.dim_users.user_id"]),
            sa.PrimaryKeyConstraint("id"),
        )
        op.create_index("ix_employee_task_logs_task_created", "employee_task_logs", ["task_pk", "created_at"], unique=False)
...
def downgrade() -> None:
    ...
    if "employee_task_participants" in existing_tables:
        op.drop_index("ix_employee_task_participants_user_role", table_name="employee_task_participants")
        op.drop_table("employee_task_participants")

    if "employee_task_logs" in existing_tables:
        op.drop_index("ix_employee_task_logs_task_created", table_name="employee_task_logs")
        op.drop_table("employee_task_logs")

    if "employee_tasks" in existing_tables:
        op.drop_index("ix_employee_tasks_created_at", table_name="employee_tasks")
        op.drop_index("ix_employee_tasks_source", table_name="employee_tasks")
        op.drop_index("ix_employee_tasks_due_at", table_name="employee_tasks")
        op.drop_index("ix_employee_tasks_owner_status", table_name="employee_tasks")
