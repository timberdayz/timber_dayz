...
def test_contract_discovery():
    ...
    assert "account_manager" in registry.list_applications(), "应该能发现 account_manager"
...
def test_contract_registration():
    ...
    # 确保应用已注册
    if "account_manager" not in registry.list_applications():
        registry.discover_applications()
    
    # 检查元数据
    info = registry.get_application_info("account_manager")
    assert info is not None, "应该能获取应用信息"
    assert info.get("name") == "账号管理", "应用名称应该正确"
...
def test_contract_health_check():
    ...
    # 获取应用实例
    app = registry.get_application("account_manager")
