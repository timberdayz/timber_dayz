...
try:
    ...
    logging.warning("⚠️ 智能代理管理器不可用，将使用静态代理配置")
...
class ShopeeCollector:
    ...
    def __init__(self, account_config: Dict[str, Any], config: Dict[str, Any] = None):
        ...
        try:
            ...
            logger.info("✅ 代理配置管理器已初始化")
        except ImportError as e:
            logger.warning(f"⚠️ 代理配置管理器不可用: {e}")
            self.proxy_config_manager = None
        
        # 保持向后兼容 - 智能代理管理器
        if PROXY_MANAGER_AVAILABLE:
            self.proxy_manager = ProxyManager()
            logger.info("✅ 智能代理管理器已初始化")
        else:
    ...
    def initialize_browser(self, headless: bool = False) -> bool:
        ...
        try:
            ...
            if proxy_config:
                ...
                logger.info(f"🌐 使用智能代理: {proxy_config['server']}")
            else:
                logger.info("🌐 使用直连模式（智能策略推荐）")
    ...
    def _handle_route(self, route):
        ...
        try:
            ...
            if should_block:
                logger.debug(f"🚫 拦截请求: {route.request.url}")
                route.abort()
            else:
                route.continue_()
    ...
    def _get_smart_proxy_config(self) -> Optional[Dict[str, str]]:
        ...
        try:
            ...
            if self.proxy_config_manager:
                logger.info("🎯 使用代理配置管理器获取代理...")
                proxy_config = self.proxy_config_manager.get_playwright_proxy_config(self.account_config)
                if proxy_config:
                    logger.info(f"✅ 代理配置管理器获取到代理: {proxy_config['server']}")
                    return proxy_config
                else:
                    logger.info("🎯 代理配置管理器建议直连模式")
            
            # 向后兼容 - 使用智能代理管理器
            if self.proxy_manager:
                logger.info("🎯 使用智能代理管理器分析最佳网络策略...")
                ...
                if strategy['use_proxy']:
                    ...
                    if proxy_info:
                        ...
                        logger.info(f"✅ 智能代理管理器获取到代理: {proxy_info.ip}:{proxy_info.port} ({proxy_info.location})")
                        return proxy_config
                    else:
                        logger.warning("⚠️ 无法获取代理，回退到直连模式")
                        return None
                else:
                    logger.info(f"🎯 智能策略: {strategy['reason']}")
                    return None
    ...
    def save_session(self) -> bool:
        ...
        try:
            if not self.context:
                logger.warning("⚠️ 没有活跃的浏览器上下文，无法保存会话")
                return False
        ...
        try:
            ...
            if session_data.get('local_storage'):
                try:
                    ...
                    logger.info("✅ LocalStorage已恢复")
                except Exception as e:
                    logger.debug(f"恢复localStorage失败: {e}")
            
            logger.info(f"✅ 会话已加载: {self.session_file}")
            return True
    ...
    def navigate_to_login(self) -> bool:
        ...
        try:
            ...
            logger.info(f"🌐 正在导航到登录页面: {self.login_url}")
            ...
            for strategy, timeout in loading_strategies:
                try:
                    logger.info(f"🔄 尝试加载策略: {strategy} (超时: {timeout}ms)")
                    ...
                    if len(input_elements) >= 2:  # 至少需要用户名和密码输入框
                        ...
                        logger.warning(f"⚠️ 页面加载不完整，未找到输入元素")
                        continue
                        
                except Exception as e:
                    logger.warning(f"⚠️ 加载策略 {strategy} 失败: {e}")
                    continue
            
            logger.error("❌ 所有加载策略都失败")
            return False
        ...
        try:
            ...
            for indicator in logged_in_indicators:
                if indicator in current_url.lower():
                    logger.info(f"✅ 检测到已登录标识: {indicator}")
                    return True
            ...
            for selector in user_indicators:
                try:
                    element = self.page.query_selector(selector)
                    if element and element.is_visible():
                        logger.info(f"✅ 发现用户信息元素: {selector}")
                        return True
                except:
                    continue
            
            logger.info("⚠️ 无法确定登录状态，默认为未登录")
            return False
    ...
    def _handle_post_login_verification(self) -> bool:
        ...
        try:
            ...
            logger.warning("⚠️ 登录状态不明确，可能需要手动处理")
            return False
            
        except Exception as e:
            logger.error(f"❌ 处理登录后验证失败: {e}")
            return False
    ...
    def _handle_email_verification(self) -> bool:
        ...
        try:
            ...
            # 这里应该集成邮箱OTP获取功能
            # 目前返回False，表示需要手动处理
            logger.warning("⚠️ 邮箱验证需要手动处理，请检查邮箱并输入验证码")
            ...
            logger.error(f"❌ 处理邮箱验证失败: {e}")
            return False
    
    def _handle_phone_verification(self) -> bool:
        """处理手机验证"""
        try:
            logger.info("📱 开始处理手机验证...")
            
            # 手机验证通常需要手动处理
            logger.warning("⚠️ 手机验证需要手动处理，请输入短信验证码")
    ...
    def _handle_captcha(self) -> bool:
        ...
        try:
            ...
            # 验证码通常需要手动处理
            logger.warning("⚠️ 验证码需要手动处理，请输入图片验证码")
        ...
        try:
            ...
            if self.load_session():
                ...
                if self.navigate_to_login():
                    if self.check_login_status():
                        logger.info("✅ 会话有效，无需重新登录")
                        return True
                    else:
                        logger.info("⚠️ 会话无效，需要重新登录")
    ...
    def collect_order_data(self, date_range: Optional[Dict[str, str]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"✅ 订单数据采集完成，共 {len(orders)} 条记录")
    ...
    def run_full_collection(self) -> Dict[str, Any]:
        ...
        try:
            logger.info("🚀 开始Shopee商家端完整数据采集流程")
    ...
    def _save_collection_record(self, collection_result: Dict, download_result: bool, start_time: datetime) -> str:
        ...
        try:
            ...
            logger.info(f"📝 采集记录已保存: {record_file}")
    ...
    def collect_operational_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info("✅ 运营数据采集完成")
            return result
            
        except Exception as e:
            logger.error(f"❌ 采集运营数据失败: {e}")
            return {'success': False, 'error': str(e)}
    
    def download_data_report(self) -> bool:
        """下载数据报告"""
        try:
            logger.info("📥 开始下载数据报告...")
            
            # 这里可以实现具体的下载逻辑
            # 目前返回True表示下载成功（占位实现）
            logger.info("✅ 数据报告下载完成")
            return True
            
        except Exception as e:
            logger.error(f"❌ 下载数据报告失败: {e}")
            return False
    ...
    def export_data_to_csv(self, data: Dict[str, Any]) -> List[str]:
        ...
        try:
            ...
            if 'financial' in data and data['financial'].get('data'):
                ...
                logger.info("✅ 财务CSV已导出")
            
            logger.info(f"✅ CSV文件导出完成: {len(csv_files)} 个文件")
            return csv_files
