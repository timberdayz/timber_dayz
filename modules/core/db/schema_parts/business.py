...
class POHeader(Base):
    ...
    vendor_code = Column(
        String(64), ForeignKey("core.dim_vendors.vendor_code"), nullable=False
    )
...
class POLine(Base):
    ...
    po_id = Column(
        String(64),
        ForeignKey("finance.po_headers.po_id", ondelete="CASCADE"),
        nullable=False,
    )
...
class GRNLine(Base):
    ...
    grn_id = Column(
        String(64),
        ForeignKey("finance.grn_headers.grn_id", ondelete="CASCADE"),
        nullable=False,
    )
    po_line_id = Column(
        Integer, ForeignKey("finance.po_lines.po_line_id"), nullable=False
    )
...
class InvoiceHeader(Base):
    ...
    source_file_id = Column(
        Integer, ForeignKey("catalog_files.id", ondelete="SET NULL"), nullable=True
    )
...
class InvoiceLine(Base):
    ...
    invoice_id = Column(
        Integer,
        ForeignKey("finance.invoice_headers.invoice_id", ondelete="CASCADE"),
        nullable=False,
    )
    po_line_id = Column(
        Integer, ForeignKey("finance.po_lines.po_line_id"), nullable=True
    )
    grn_line_id = Column(
        Integer, ForeignKey("finance.grn_lines.grn_line_id"), nullable=True
    )
...
class ThreeWayMatchLog(Base):
    ...
    invoice_line_id = Column(
        Integer, ForeignKey("finance.invoice_lines.invoice_line_id"), nullable=True
    )

    match_status = Column(
        String(32), default="unmatched", nullable=False
...
class FactExpensesMonth(Base):
    ...
    period_month = Column(
        String(16), ForeignKey("core.dim_fiscal_calendar.period_code"), nullable=False
    )
...
class AllocationRule(Base):
    ...
    )  # revenue_share/orders_share/units_share/weight/volume/manual
...
class FactExpensesAllocated(Base):
    ...
    expense_id = Column(
        Integer, ForeignKey("finance.fact_expenses_month.expense_id"), nullable=False
    )
    ...
    __table_args__ = (
        ...
        Index(
            "ix_expenses_allocated_shop", "platform_code", "shop_id", "allocation_date"
        ),
        Index(
            "ix_expenses_allocated_sku",
            "platform_code",
            "shop_id",
            "platform_sku",
            "allocation_date",
...
class LogisticsCost(Base):
    ...
    grn_id = Column(
        String(64), ForeignKey("finance.grn_headers.grn_id"), nullable=True
    )  # 入库物流
    ...
    invoice_id = Column(
        Integer, ForeignKey("finance.invoice_headers.invoice_id"), nullable=True
    )
...
class WarehouseStorageRule(Base):
    ...
    label_fee = Column(Numeric(18, 2), nullable=True)
    effective_from = Column(Date, nullable=False)
    effective_to = Column(Date, nullable=True)
    status = Column(String(16), nullable=False, default="active")
    source = Column(String(128), nullable=True)
...
class LogisticsBillLine(Base):
    ...
    actual_total_weight_kg = Column(Numeric(18, 3), nullable=True)
    actual_total_volume_cbm = Column(Numeric(18, 6), nullable=True)
    headhaul_cost = Column(Numeric(18, 2), nullable=True)
    handling_cost = Column(Numeric(18, 2), nullable=True)
    last_mile_cost = Column(Numeric(18, 2), nullable=True)
    unit_headhaul_cost = Column(Numeric(18, 6), nullable=True)
    unit_handling_cost = Column(Numeric(18, 6), nullable=True)
    unit_last_mile_cost = Column(Numeric(18, 6), nullable=True)
    notes = Column(Text, nullable=True)
...
class SkuProfitEstimate(Base):
    ...
    logistics_cost_source = Column(String(64), nullable=True)
    storage_cost = Column(Numeric(18, 2), nullable=True)
    storage_cost_source = Column(String(64), nullable=True)
    assumption_profile_id = Column(Integer, ForeignKey("finance.product_cost_assumption_profiles.profile_id", ondelete="SET NULL"), nullable=True)
    ...
    expected_label_fee = Column(Numeric(18, 2), nullable=True)
...
class JournalEntryLine(Base):
    ...
    entry_id = Column(
        Integer,
        ForeignKey("finance.journal_entries.entry_id", ondelete="CASCADE"),
        nullable=False,
    )
    line_number = Column(Integer, nullable=False)

    account_code = Column(
        String(64), ForeignKey("finance.gl_accounts.account_code"), nullable=False
    )
...
class InventoryLayerConsumption(Base):
    ...
    __table_args__ = (
        ...
        Index(
            "ix_inventory_layer_consumptions_sku",
            "platform_code",
            "shop_id",
            "platform_sku",
        ),
...
class InventoryAdjustmentLine(Base):
    ...
    adjustment_id = Column(
        String(64),
        ForeignKey(
            "finance.inventory_adjustment_headers.adjustment_id", ondelete="CASCADE"
        ),
        nullable=False,
    )
    platform_code = Column(String(32), nullable=False)
    ...
    __table_args__ = (
        ...
        Index(
            "ix_inventory_adjustment_lines_sku",
            "platform_code",
            "shop_id",
            "platform_sku",
        ),
...
class EmployeeLaborCostAllocation(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "allocation_scope IN ('shop', 'company')",
            name="ck_employee_labor_cost_scope",
        ),
        Index("ix_employee_labor_cost_period", "period_month"),
        Index("ix_employee_labor_cost_employee", "employee_code", "period_month"),
        Index(
            "ix_employee_labor_cost_shop", "platform_code", "shop_id", "period_month"
        ),
        Index(
...
class FollowInvestment(Base):
    ...
    investor_user_id = Column(
        BigInteger, ForeignKey("core.dim_users.user_id"), nullable=False
    )
    platform_code = Column(String(32), nullable=False)
    shop_id = Column(String(64), nullable=False)
...
class FollowInvestmentSettlement(Base):
    ...
    profit_basis_id = Column(
        Integer, ForeignKey("finance.shop_profit_basis.id"), nullable=False
    )
...
class FollowInvestmentDetail(Base):
    ...
    settlement_id = Column(
        Integer,
        ForeignKey("finance.follow_investment_settlements.id", ondelete="CASCADE"),
        nullable=False,
    )
    investor_user_id = Column(
        BigInteger, ForeignKey("core.dim_users.user_id"), nullable=False
...
class MonthlyProfitPersonnelDetail(Base):
    ...
    settlement_id = Column(
        Integer,
        ForeignKey("finance.monthly_profit_settlements.id", ondelete="CASCADE"),
        nullable=False,
    )
...
class MonthlyProfitFollowDetail(Base):
    ...
    investor_user_id = Column(
        BigInteger, ForeignKey("core.dim_users.user_id"), nullable=True
    )
    source_settlement_id = Column(
        Integer, ForeignKey("finance.follow_investment_settlements.id"), nullable=True
    )
    amount = Column(Float, default=0.0, nullable=False)
...
class SalesCampaign(Base):
    ...
    campaign_type = Column(
        String(32), nullable=False, comment="战役类型:holiday/new_product/special_event"
    )
    ...
    target_amount = Column(
        Float, nullable=False, default=0.0, comment="目标销售额(CNY)"
    )
    target_quantity = Column(
        Integer, nullable=False, default=0, comment="目标订单数/销量"
    )

    # 达成值(C类:系统自动计算)
    actual_amount = Column(
        Float, nullable=False, default=0.0, comment="实际销售额(CNY)"
    )
    actual_quantity = Column(
        Integer, nullable=False, default=0, comment="实际订单数/销量"
    )
    achievement_rate = Column(
        Float, nullable=False, default=0.0, comment="达成率(百分比)"
    )
    ...
    status = Column(
        ...
        nullable=False,
        default="pending",
        comment="状态:active/completed/pending/cancelled",
    )
...
class SalesCampaignShop(Base):
    ...
    campaign_id = Column(
        Integer,
        ForeignKey("a_class.sales_campaigns.id", ondelete="CASCADE"),
        nullable=False,
    )
    platform_code = Column(String(32), nullable=True)
    shop_id = Column(String(64), nullable=True)
    ...
    __table_args__ = (
        UniqueConstraint(
            "campaign_id", "platform_code", "shop_id", name="uq_campaign_shop"
        ),
        ForeignKeyConstraint(
...
class SalesTarget(Base):
    ...
    target_type = Column(
        String(32), nullable=False, comment="目标类型:shop/product/campaign/operation"
    )
    ...
    target_profit_amount = Column(
        Float, nullable=False, default=0.0, comment="目标毛利(CNY)"
    )
    target_profit_basis_amount = Column(
        Float, nullable=False, default=0.0, comment="目标结算利润基数(CNY)"
    )
    ...
    # 达成值(C类:系统自动计算)
    achieved_amount = Column(
        Float, nullable=False, default=0.0, comment="实际销售额(CNY)"
    )
    achieved_quantity = Column(
        Integer, nullable=False, default=0, comment="实际订单数/销量"
    )
    achieved_profit_amount = Column(
        Float, nullable=False, default=0.0, comment="实际毛利(CNY)"
    )
    achievement_rate = Column(
        Float, nullable=False, default=0.0, comment="达成率(百分比)"
    )
    ...
    metric_direction = Column(
        String(32),
        nullable=True,
        comment="指标方向:higher_better/lower_better/manual_score",
    )
    ...
    penalty_enabled = Column(
        Boolean, nullable=False, default=False, comment="是否启用罚分"
    )
    ...
    manual_score_enabled = Column(
        Boolean, nullable=False, default=False, comment="是否允许人工打分"
    )
    manual_score_value = Column(Float, nullable=True, comment="人工打分值")
    is_enabled = Column(
        Boolean, nullable=False, default=True, comment="运营指标是否参与当月计算"
    )
    metric_catalog_version = Column(
        Integer, nullable=True, comment="运营指标目录版本快照"
    )
    performance_config_id = Column(
        BigInteger, nullable=True, comment="保存时生效的绩效配置ID"
    )
    ...
    weekday_ratios = Column(
        JSON, nullable=True, comment='周一到周日拆分比例 {"1":0.14,...,"7":0.14} 和为1'
    )
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "target_profit_basis_amount >= 0", name="chk_target_profit_basis_amount"
        ),
...
class TargetBreakdown(Base):
    ...
    target_id = Column(
        Integer,
        ForeignKey("a_class.sales_targets.id", ondelete="CASCADE"),
        nullable=False,
    )
    ...
    period_label = Column(
        String(64), nullable=True, comment="周期标签,如'第1周'、'2025-01'"
    )
    ...
    __table_args__ = (
        CheckConstraint(
            "breakdown_type IN ('shop', 'time', 'shop_time')", name="chk_breakdown_type"
        ),
...
class ShopHealthScore(Base):
    ...
    granularity = Column(
        String(16), nullable=False, default="daily", comment="粒度:daily/weekly/monthly"
    )

    # 健康度总分(0-100)
    health_score = Column(
        Float, nullable=False, default=0.0, comment="健康度总分(0-100)"
    )
    ...
    conversion_rate = Column(
        Float, nullable=False, default=0.0, comment="转化率(百分比)"
    )
    inventory_turnover = Column(
        Float, nullable=False, default=0.0, comment="库存周转率"
    )
    customer_satisfaction = Column(
        Float, nullable=False, default=0.0, comment="客户满意度(0-5分)"
    )

    # 风险等级
    risk_level = Column(
        String(16), nullable=False, default="low", comment="风险等级:low/medium/high"
    )
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "health_score >= 0 AND health_score <= 100", name="chk_health_score"
        ),
        CheckConstraint(
            "risk_level IN ('low', 'medium', 'high')", name="chk_risk_level"
        ),
...
class ShopAlert(Base):
    ...
    alert_type = Column(
        String(64),
        nullable=False,
        comment="预警类型:inventory_turnover/conversion_rate/gmv_drop/...",
    )
    alert_level = Column(
        String(16), nullable=False, comment="预警级别:critical/warning/info"
    )
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "alert_level IN ('critical', 'warning', 'info')", name="chk_alert_level"
        ),
...
class PerformanceScore(Base):
    ...
    # 各项得分(权重 × 达成率)
    sales_score = Column(
        Float, nullable=False, default=0.0, comment="销售额得分(权重30%)"
    )
    profit_score = Column(
        Float, nullable=False, default=0.0, comment="毛利得分(权重25%)"
    )
    key_product_score = Column(
        Float, nullable=False, default=0.0, comment="重点产品得分(权重25%)"
    )
    operation_score = Column(
        Float, nullable=False, default=0.0, comment="运营得分(权重20%)"
    )
    ...
    # 排名和系数
    rank = Column(Integer, nullable=True, comment="排名")
    performance_coefficient = Column(
        Float, nullable=True, comment="绩效系数；仅正式结算结果写入"
    )
    ...
    __table_args__ = (
        UniqueConstraint(
            "platform_code", "shop_id", "period", name="uq_performance_shop_period"
        ),
        ForeignKeyConstraint(
            ["platform_code", "shop_id"],
            ["core.dim_shops.platform_code", "core.dim_shops.shop_id"],
        ...
        CheckConstraint(
            "total_score >= 0 AND total_score <= 100", name="chk_total_score"
        ),
...
class PerformanceConfig(Base):
    ...
    # 配置信息
    config_name = Column(
        String(64), nullable=False, default="default", comment="配置名称"
    )
    ...
    key_product_weight = Column(
        Integer, nullable=False, default=0, comment="重点产品权重(%)，当前暂不启用"
    )
    operation_weight = Column(
        Integer, nullable=False, default=20, comment="运营权重(%)"
    )
    ...
    key_product_max_score = Column(
        Integer, nullable=False, default=0, comment="重点产品满分，当前暂不启用"
    )
    operation_max_score = Column(
        Integer, nullable=False, default=20, comment="运营满分"
    )
...
class ClearanceRanking(Base):
    ...
    # 清理数据
    clearance_amount = Column(
        Float, nullable=False, default=0.0, comment="清理金额(CNY)"
    )
    clearance_quantity = Column(Integer, nullable=False, default=0, comment="清理数量")

    # 激励金额
    incentive_amount = Column(
        Float, nullable=False, default=0.0, comment="激励金额(CNY)"
    )
    total_incentive = Column(
        Float, nullable=False, default=0.0, comment="总计激励(CNY)"
    )
...
class MaterializedViewRefreshLog(Base):
    ...
    refresh_started_at = Column(
        DateTime(timezone=True),
        server_default=func.now(),
        nullable=False,
        comment="刷新开始时间",
    )
    refresh_completed_at = Column(
        DateTime(timezone=True), nullable=True, comment="刷新完成时间"
    )
    ...
    __table_args__ = (
        CheckConstraint(
            "status IN ('running', 'success', 'failed')", name="chk_mv_refresh_status"
        ),
...
user_roles = Table(
    ...
    Base.metadata,
    Column(
        "user_id", BigInteger, ForeignKey("core.dim_users.user_id"), primary_key=True
    ),
    Column(
        "role_id", BigInteger, ForeignKey("core.dim_roles.role_id"), primary_key=True
    ),
...
class DimUser(Base):
    ...
    # 关系
    roles = relationship("DimRole", secondary=user_roles, back_populates="users")
    audit_logs = relationship(
        "FactAuditLog", back_populates="user", cascade="all, delete-orphan"
    )
...
class DimRole(Base):
    ...
    )  # 角色代码,如:admin, finance, warehouse
    description = Column(Text)

    # 权限(JSON格式)
    permissions = Column(
        Text, nullable=False
    )  # JSON数组,如:["view_sales", "edit_inventory"]
    ...
    # 关系
    users = relationship("DimUser", secondary=user_roles, back_populates="roles")
...
class UserSession(Base):
    ...
    session_id = Column(
        String(64), primary_key=True, index=True, comment="会话ID(Token的哈希值)"
    )
    user_id = Column(
        BigInteger, ForeignKey("core.dim_users.user_id"), nullable=False, index=True
    )
...
class UserApprovalLog(Base):
    ...
    reason = Column(Text, nullable=True, comment="操作原因/备注")
...
class FactAuditLog(Base):
    ...
    )  # order/product/inventory/finance
...
class SalesTargetA(Base):
    ...
    )  # 迁移时将重命名为"年月",格式:'2025-01'
    target_sales_amount = Column(
    ...
    __table_args__ = (
        UniqueConstraint(
            ...
            "shop_id",
            "year_month",
            name="uq_sales_targets_a_platform_shop_month",
        ),
...
class OperatingCost(Base):
    ...
    attachments = Column(
        "附件", JSONB, nullable=False, server_default=text("'[]'::jsonb")
    )
    ...
    created_at = Column(
        "创建时间", DateTime(timezone=True), server_default=func.now(), nullable=False
    )
    updated_at = Column(
        "更新时间",
        DateTime(timezone=True),
    ...
    __table_args__ = (
        UniqueConstraint(
            ...
            "店铺ID",
            "年月",
            name="uq_operating_costs_a_platform_shop_month",
        ),
...
class Employee(Base):
    ...
    id_type = Column(
        String(32), nullable=True, default="id_card"
    ...
    )  # 合同类型:fixed_term/indefinite/internship/part_time
    ...
    status = Column(
        String(32), nullable=False, default="active"
    )  # 状态:active/inactive/probation/leave
...
class EmployeeShopAssignment(Base):
    ...
    employee_code = Column(
        String(64), nullable=False
    )  # 员工编号，逻辑引用 a_class.employees
    platform_code = Column(String(32), nullable=False)  # 平台编码
    shop_id = Column(String(256), nullable=False)  # 店铺ID

    commission_ratio = Column(
        Float, nullable=True
    )  # 提成比例(0-1)，NULL时使用薪资结构默认
    target_allocation_ratio = Column(
        Float, nullable=False, default=1.0, server_default="1.0"
    )  # 目标分配比例(0-1)
    target_allocation_ratio_source = Column(
    ...
    __table_args__ = (
        UniqueConstraint(
            ...
            "shop_id",
            "year_month",
            name="uq_employee_shop_assignments_a",
        ),
        ForeignKeyConstraint(
            ["platform_code", "shop_id"],
            ["core.dim_shops.platform_code", "core.dim_shops.shop_id"],
...
class ShopCommissionConfig(Base):
    ...
    year_month = Column(
        String(7), nullable=False
    )  # YYYY-MM，与 employee_shop_assignments 一致
    platform_code = Column(String(32), nullable=False)
    shop_id = Column(String(256), nullable=False)
    allocatable_profit_rate = Column(
    ...
    __table_args__ = (
        UniqueConstraint(
            "year_month", "platform_code", "shop_id", name="uq_shop_commission_config_a"
        ),
        ForeignKeyConstraint(
            ["platform_code", "shop_id"],
            ["core.dim_shops.platform_code", "core.dim_shops.shop_id"],
...
class AttendanceRecord(Base):
    ...
    clock_in_type = Column(
        String(32), nullable=True, default="normal"
    )  # 打卡类型:normal/field/supplement
    clock_out_type = Column(
        String(32), nullable=True, default="normal"
    )  # 打卡类型:normal/field/supplement
    ...
    __table_args__ = (
        UniqueConstraint(
            "employee_code", "attendance_date", name="uq_attendance_records_a"
        ),
...
class OvertimeRecord(Base):
    ...
    overtime_type = Column(
        String(32), nullable=False, default="workday"
    )  # 类型:workday/weekend/holiday
...
class SalaryStructure(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "employee_code",
            "effective_date",
            name="uq_salary_structures_employee_effective_date",
        ),
...
class PayrollRecord(Base):
    ...
    # === 扣除项 ===
    social_insurance_personal = Column(
        Numeric(15, 2), nullable=False, default=0.0
    )  # 社保个人部分
    housing_fund_personal = Column(
        Numeric(15, 2), nullable=False, default=0.0
    )  # 公积金个人部分
    ...
    # === 公司成本 ===
    social_insurance_company = Column(
        Numeric(15, 2), nullable=False, default=0.0
    )  # 社保公司部分
    housing_fund_company = Column(
        Numeric(15, 2), nullable=False, default=0.0
    )  # 公积金公司部分
...
class SocialInsuranceConfig(Base):
    ...
    # === 失业保险 ===
    unemployment_company_ratio = Column(
        Float, nullable=False, default=0.008
    )  # 单位缴纳比例
    unemployment_personal_ratio = Column(
        Float, nullable=False, default=0.002
    )  # 个人缴纳比例

    # === 工伤保险 ===
    injury_company_ratio = Column(
        Float, nullable=False, default=0.002
    )  # 单位缴纳比例(个人不缴)

    # === 生育保险 ===
    maternity_company_ratio = Column(
        Float, nullable=False, default=0.008
    )  # 单位缴纳比例(个人不缴)

    # === 住房公积金 ===
    housing_fund_company_ratio = Column(
        Float, nullable=False, default=0.12
    )  # 单位缴纳比例
    housing_fund_personal_ratio = Column(
        Float, nullable=False, default=0.12
    )  # 个人缴纳比例
...
class EmployeePerformance(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "employee_code", "year_month", name="uq_employee_performance_c"
        ),
...
class EmployeeCommission(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "employee_code", "year_month", name="uq_employee_commissions_c"
        ),
...
class Notification(Base):
    ...
    recipient = relationship(
        "DimUser", foreign_keys=[recipient_id], backref="notifications"
    )
...
class UserNotificationPreference(Base):
    ...
    enabled = Column(
        Boolean, default=True, nullable=False, comment="是否启用通知(应用内通知)"
    )
    ...
    # 关系
    user = relationship(
        "DimUser", foreign_keys=[user_id], backref="notification_preferences"
    )
...
class DimRateLimitConfig(Base):
    ...
    role_code = Column(
        String(50), nullable=False, index=True
    )  # admin/manager/finance/operator/normal/anonymous
    endpoint_type = Column(
        String(50), nullable=False, index=True
    )  # default/data_sync/auth
    ...
    __table_args__ = (
        UniqueConstraint(
            "role_code", "endpoint_type", name="uq_rate_limit_config_role_endpoint"
        ),
...
class FactRateLimitConfigAudit(Base):
    ...
    # 配置信息
    config_id = Column(
        Integer, ForeignKey("dim_rate_limit_config.config_id"), nullable=True
    )  # 配置ID(删除时为NULL)
...
class SystemLog(Base):
    ...
    __table_args__ = (
        ...
        ForeignKeyConstraint(
            ["user_id"], ["core.dim_users.user_id"], name="fk_system_logs_user_id"
        ),
    )
...
class SecurityConfig(Base):
    ...
    )  # password_policy, login_restrictions, session_config, 2fa_config
    ...
    __table_args__ = (
        ...
        ForeignKeyConstraint(
            ...
            name="fk_security_config_updated_by",
        ),
    )
...
class BackupRecord(Base):
    ...
    __table_args__ = (
        ...
        ForeignKeyConstraint(
            ["created_by"],
            ["core.dim_users.user_id"],
            name="fk_backup_records_created_by",
        ),
        {"schema": "core"},
    )
...
class SMTPConfig(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ["updated_by"], ["core.dim_users.user_id"], name="fk_smtp_config_updated_by"
        ),
    )
...
class AlertRule(Base):
    ...
    rule_type = Column(
        String(64), nullable=False
    )  # system/performance/security/business
    ...
    __table_args__ = (
        ...
        ForeignKeyConstraint(
            ...
            name="fk_alert_rules_template_id",
        ),
        ForeignKeyConstraint(
            ["created_by"], ["core.dim_users.user_id"], name="fk_alert_rules_created_by"
        ),
        ForeignKeyConstraint(
            ["updated_by"], ["core.dim_users.user_id"], name="fk_alert_rules_updated_by"
        ),
    )
...
class SystemConfig(Base):
    ...
    config_key = Column(
        String(64), unique=True, nullable=False
    )  # system_name, version, timezone, language, currency
    ...
    __table_args__ = (
        ...
        ForeignKeyConstraint(
            ["updated_by"],
            ["core.dim_users.user_id"],
            name="fk_system_config_updated_by",
        ),
    )
