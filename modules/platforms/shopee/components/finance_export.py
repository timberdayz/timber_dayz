...
class ShopeeFinanceExport(ExportComponent):
    ...
    def __init__(self, ctx: ExecutionContext, selectors: Optional[FinanceSelectors] = None) -> None:
        super().__init__(ctx)
        self.sel = selectors or FinanceSelectors()
    ...
    async def run(self, page: Any, mode: ExportMode = ExportMode.STANDARD) -> ExportResult:  # type: ignore[override]
        try:
            ...
            try:
                ...
                if preset:
                    ...
                    if not ok and self.logger:
                        self.logger.warning("[ShopeeFinanceExport] 日期设置未生效，将继续尝试导出")
            except Exception:
                pass
            ...
            for btn in self.sel.export_buttons:
                try:
                    ...
                    if await loc.count() > 0 and await loc.first.is_visible():
                        if self.logger:
                            self.logger.info(f"[ShopeeFinanceExport] 点击导出按钮: {btn}")
                ...
                try:
                    ...
                    if await loc.count() > 0 and await loc.first.is_visible():
                        try:
                            ...
                            if self.logger:
                                self.logger.info(f"[ShopeeFinanceExport] 立即下载成功")
                            break
                        except Exception:
                            if self.logger:
                                self.logger.info(f"[ShopeeFinanceExport] 未检测到立即下载，尝试重试机制")
                                ...
                                for export_btn in self.sel.export_buttons:
                                    try:
                                        ...
                                        if await export_loc.count() > 0 and await export_loc.first.is_visible():
                                            ...
                                            if self.logger:
                                                self.logger.info(f"[ShopeeFinanceExport] 30s后重试点击导出按钮")
                                            await page.wait_for_timeout(1000)
                                            break
                                    except Exception:
                                        continue
            ...
            if download:
                ...
                out_root = build_standard_output_root(self.ctx, data_type="finance", granularity=gran)
                out_root.mkdir(parents=True, exist_ok=True)

                tmp_name = download.suggested_filename or "finance.xlsx"
                ...
                if self.logger:
                    self.logger.success(f"[OK] Export successful: {target}")
                return ExportResult(True, str(target), "下载完成(UI)")
            else:
                return ExportResult(False, None, "未捕获到下载事件或未找到下载按钮")

        except Exception as e:
            if self.logger:
                self.logger.error(f"[ShopeeFinanceExport] 失败: {e}")
