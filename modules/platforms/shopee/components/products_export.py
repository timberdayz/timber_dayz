...
class ShopeeProductsExport(ExportComponent):
    ...
    def _region_text_candidates(self, region_value: str | None) -> tuple[str, ...]:
        return self._shop_switch_helper()._region_text_candidates(region_value)
    ...
    def _shop_name_looks_selected(
        ...
        return self._shop_switch_helper()._shop_name_looks_selected(
            current_label,
            str(target_shop or ""),
            target_region,
        )
    ...
    def _date_trigger_fallback_texts(self) -> tuple[str, ...]:
        return self._date_picker_helper()._date_trigger_fallback_texts()

    async def _find_date_picker_trigger(self, page: Any) -> Any | None:
        return await self._date_picker_helper()._find_date_picker_trigger(page)

    async def _find_date_summary_container(self, page: Any) -> Any | None:
        return await self._date_picker_helper()._find_date_summary_container(page)

    async def _find_date_panel(self, page: Any) -> Any | None:
        return await self._date_picker_helper()._find_date_panel(page)

    async def _find_date_option_locator(self, page: Any, text: str) -> Any | None:
        return await self._date_picker_helper()._find_date_option_locator(page, text)

    async def _hover_text_option(self, page: Any, text: str) -> bool:
        return await self._date_picker_helper()._hover_text_option(page, text)

    def _match_known_date_label(self, value: str | None) -> str | None:
        return self._date_picker_helper()._match_known_date_label(value)

    def _preset_value_from_label(self, label: str | None) -> str | None:
        return self._date_picker_helper()._preset_value_from_label(label)

    def _parse_summary_date_value(self, value: str | None) -> str | None:
        return self._date_picker_helper()._parse_summary_date_value(value)

    def _extract_summary_date_values(self, summary: str | None) -> tuple[str, ...]:
        return self._date_picker_helper()._extract_summary_date_values(summary)

    def _extract_summary_month_value(self, summary: str | None) -> tuple[int, int] | None:
        return self._date_picker_helper()._extract_summary_month_value(summary)

    def _parse_date_summary(self, summary: str | None) -> dict[str, Any] | None:
        return self._date_picker_helper()._parse_date_summary(summary)
    ...
    def _custom_time_selection(self, config: dict[str, Any]) -> dict[str, Any] | None:
        return self._date_picker_helper()._custom_time_selection(config)
    ...
    def _resolve_custom_target(self, config: dict[str, Any]) -> dict[str, Any]:
        return self._date_picker_helper()._resolve_custom_target(config)

    async def _find_month_panel(self, page: Any) -> Any | None:
        return await self._date_picker_helper()._find_month_panel(page)

    async def _find_month_panel_header(self, page: Any) -> Any | None:
        return await self._date_picker_helper()._find_month_panel_header(page)

    async def _current_month_panel_year_text(self, page: Any) -> str | None:
        return await self._date_picker_helper()._current_month_panel_year_text(page)
    ...
    async def _wait_month_panel_year_changed(
        ...
        *,
        ...
        return await self._date_picker_helper()._wait_month_panel_year_changed(
            page,
            previous_header,
    ...
    async def _click_month_panel_year_nav_button(self, page: Any, direction: str) -> bool:
        return await self._date_picker_helper()._click_month_panel_year_nav_button(page, direction)

    async def _find_month_grid_cell(self, page: Any, month_label: str) -> Any | None:
        return await self._date_picker_helper()._find_month_grid_cell(page, month_label)
    ...
    def _parse_calendar_header_month_year_zh(self, header_text: str | None) -> tuple[int, int] | None:
        return self._date_picker_helper()._parse_calendar_header_month_year_zh(header_text)
    ...
    def _parse_month_panel_year(self, header_text: str | None) -> int | None:
        return self._date_picker_helper()._parse_month_panel_year(header_text)

    def _normalize_custom_granularity(self, granularity: str | None) -> str:
        return self._date_picker_helper()._normalize_custom_granularity(granularity)
    ...
    async def _navigate_month_panel_to_year(self, page: Any, target_year: int) -> bool:
        return await self._date_picker_helper()._navigate_month_panel_to_year(page, target_year)

    async def _navigate_calendar_panel_to_month(self, page: Any, target_year: int, target_month: int) -> bool:
        return await self._date_picker_helper()._navigate_calendar_panel_to_month(page, target_year, target_month)

    async def _current_date_summary_text(self, page: Any) -> str | None:
        return await self._date_picker_helper()._current_date_summary_text(page)
    ...
    async def _wait_shop_selection_applied(
        ...
        *,
        ...
        return await self._shop_switch_helper()._wait_shop_selection_applied(
            page,
            target_shop,
            target_region,
            timeout_ms=timeout_ms,
    ...
    async def _ensure_shop_selected(self, page: Any) -> None:
        result = await self._shop_switch_helper().run(page)
        if not result.success:
            raise RuntimeError(result.message or "shop switch failed")

    async def _current_date_label(self, page: Any) -> str | None:
        return await self._date_picker_helper()._current_date_label(page)

    def _target_date_label(self, config: dict[str, Any]) -> str:
        return self._date_picker_helper()._target_date_label(config)

    async def _ensure_date_selection(self, page: Any) -> None:
        picker = self._date_picker_helper()
        result = await picker.run(page, picker._resolve_option_from_context())
        if not result.success:
            raise RuntimeError(result.message or "date selection failed")
    ...
    async def _select_single_day_value(self, page: Any, iso_date: str) -> bool:
        return await self._date_picker_helper()._select_single_day_value(page, iso_date)

    async def _select_calendar_day(self, page: Any, day: int) -> bool:
        return await self._date_picker_helper()._select_calendar_day(page, day)

    async def _select_month_value(self, page: Any, iso_date: str) -> bool:
        return await self._date_picker_helper()._select_month_value(page, iso_date)

    async def _select_week_range_value(self, page: Any, start_date: str, end_date: str) -> bool:
        return await self._date_picker_helper()._select_week_range_value(page, start_date, end_date)
