...
class TiktokAnalyticsExport(ExportComponent):
    ...
    async def run(self, page: Any, mode: ExportMode = ExportMode.STANDARD) -> ExportResult:  # type: ignore[override]
        ...
        if entry_state == "login":
            return ExportResult(success=False, message="login required before analytics export", file_path=None)
        ...
        if not getattr(export_result, "success", False):
            return ExportResult(
                success=False,
                message=getattr(export_result, "message", "analytics export failed"),
                file_path=None,
            )
