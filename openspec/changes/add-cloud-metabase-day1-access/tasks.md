...
- [ ] 1.1 在 `docs/deployment/` 下新增或更新文档，包含「云端 Metabase 访问与首次初始化」小节（可新文件 `CLOUD_METABASE_ACCESS.md` 或合并进 `CLOUD_UPDATE_AND_LOCAL_VERIFICATION.md`）。
...
- [ ] 2.1 在部署检查清单或 `scripts/metabase_nginx_deployment_summary.md` 中增加一项：确认运维已知「通过 /metabase/ 访问」及「首次需完成 Metabase 初始化与 API Key 配置」。
- [ ] 2.2 检查清单中增加一项：确认 Nginx 中所有使用变量的 `proxy_pass` 都搭配了正确的 `rewrite` 规则。
