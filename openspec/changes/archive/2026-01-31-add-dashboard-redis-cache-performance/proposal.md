...
- [x] 业务概览各 Dashboard 接口支持 Redis 缓存，相同参数在 TTL 内命中缓存
- [x] 仅缓存成功响应，错误/异常不写入缓存
- [x] Redis 不可用时，接口仍可正常工作（降级为直接查询）
- [x] 2 核 4G 环境下 workers 保持 2，不增加 OOM 风险
- [x] 验证：KPI 接口命中缓存时加速约 130x，X-Cache 响应头可观测
