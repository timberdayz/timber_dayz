...
- **以店铺为行**：直接显示可用店铺（来自 `api.getTargetShops()`，与账号管理一致）；左列固定（平台、店铺）只读，右列可编辑
- **操作栏**：月份选择器、为所有店铺添加、批量保存、刷新（与费用管理布局一致）
- **列**：平台、店铺（只读）、可分配利润率、主管（多选 + 每人提成比、+ 添加）、操作员（同上）、状态（已配置/未配置）、操作（保存、删除）
- **行内编辑**：主管/操作员通过「+ 添加」弹窗选择员工和提成比，添加后为本地状态，点击「保存」提交；「删除」清除该店铺归属配置，可重新编辑
