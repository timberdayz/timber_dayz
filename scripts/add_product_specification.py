...
# -*- coding: utf-8 -*-
"""
添加产品规格字段（使用product_specification避免冲突）
"""
...
def main():
    print("=== Adding Product Specification Field ===\n")
    ...
    with engine.connect() as conn:
        ...
        try:
            ...
            # Step 2: Add product_specification mapping
            print("\n--- Step 2: Adding product_specification mapping ---")
            
            check_result = conn.execute(text("""
                SELECT id
                FROM field_mapping_dictionary
                WHERE field_code = 'product_specification';
            """))
            
            if check_result.fetchone():
                print("  [SKIP] product_specification already exists")
            else:
                synonyms = [
                    "规格", "*规格", "产品规格", "商品规格",
                ...
                conn.execute(text(f"""
                    ...
                    VALUES
                    ('product_specification', '规格', 'Specification', 'products', 'dimension',
                     false, 'string', '产品规格描述（颜色、尺寸等），如：silver S 35cmX5cm', 
                     '{synonyms_json}'::jsonb,
                     true, 'system', NOW())
                """))
                print("  [ADDED] product_specification mapping")
            
            # Step 3: Delete c68_c84_1
            print("\n--- Step 3: Deleting c68_c84_1 mapping ---")
            check_result = conn.execute(text("""
                SELECT id, cn_name
                FROM field_mapping_dictionary
            ...
            trans.commit()
            print("\n[OK] Product specification field added successfully.")
