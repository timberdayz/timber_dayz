...
mapped_monthly_traffic AS (
    SELECT
        ...
        END AS page_views,
        CASE
            WHEN COALESCE(raw_data->>'订单数', raw_data->>'订单数量', raw_data->>'order_count', raw_data->>'Order Count') IS NULL THEN NULL
            ELSE NULLIF(
                REGEXP_REPLACE(
                    REPLACE(REPLACE(REPLACE(REPLACE(
                        COALESCE(raw_data->>'订单数', raw_data->>'订单数量', raw_data->>'order_count', raw_data->>'Order Count'),
                        ',', ''
                    ), ' ', ''), CHR(8212), ''), CHR(8211), ''),
                    '[^0-9.-]',
                    '',
        ...
        END AS order_count,
        CASE
            WHEN COALESCE(raw_data->>'SKU 订单数', raw_data->>'sku_order_count', raw_data->>'SKU Order Count') IS NULL THEN NULL
            ELSE NULLIF(
                REGEXP_REPLACE(
                    REPLACE(REPLACE(REPLACE(REPLACE(
                        COALESCE(raw_data->>'SKU 订单数', raw_data->>'sku_order_count', raw_data->>'SKU Order Count'),
                        ',', ''
                    ), ' ', ''), CHR(8212), ''), CHR(8211), ''),
                    '[^0-9.-]',
                    '',
        ...
        CASE
            WHEN COALESCE(raw_data->>'GMV', raw_data->>'gmv') IS NULL THEN NULL
            ELSE NULLIF(
                REGEXP_REPLACE(
                    REPLACE(REPLACE(REPLACE(REPLACE(
                        COALESCE(raw_data->>'GMV', raw_data->>'gmv'),
                        ',', ''
                    ), ' ', ''), CHR(8212), ''), CHR(8211), ''),
                    '[^0-9.-]',
                    '',
        ...
        END AS gmv,
        CASE
            WHEN COALESCE(raw_data->>'总成交额', raw_data->>'成交金额', raw_data->>'sales_amount') IS NULL THEN NULL
            ELSE NULLIF(
                REGEXP_REPLACE(
                    REPLACE(REPLACE(REPLACE(REPLACE(
                        COALESCE(raw_data->>'总成交额', raw_data->>'成交金额', raw_data->>'sales_amount'),
                        ',', ''
                    ), ' ', ''), CHR(8212), ''), CHR(8211), ''),
                    '[^0-9.-]',
                    '',
