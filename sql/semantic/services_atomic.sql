...
mapped AS (
    SELECT
        ...
        COALESCE(raw_data->>'订单', raw_data->>'orders', raw_data->>'Orders', raw_data->>'订单数') AS order_count_raw,
        COALESCE(raw_data->>'买家数', raw_data->>'buyer_count', raw_data->>'Buyer Count') AS buyer_count_raw,
...
cleaned AS (
    SELECT
        ...
        CASE WHEN buyer_count_raw IS NULL THEN NULL ELSE NULLIF(REGEXP_REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(buyer_count_raw, ',', ''), ' ', ''), CHR(8212), ''), CHR(8211), ''), '[^0-9.-]', '', 'g'), '')::numeric END AS buyer_count,
